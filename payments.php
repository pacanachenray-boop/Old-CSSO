...
if (!empty($school_year_filter) && !empty($semester_filter)) {
    $payment_where = " AND r.school_year = '$school_year_filter' AND r.semester = '$semester_filter'";
} elseif (!empty($school_year_filter)) {
    $payment_where = " AND r.school_year = '$school_year_filter'";
} elseif (!empty($semester_filter)) {
    $payment_where = " AND r.semester = '$semester_filter'";
}
...
$sql = "SELECT 
    ...
    sp.YearLevel,
    fp.payment_type,
    SUM(fp.payment_amount) as TotalPayments,
    SUM(fp.balance) as TotalBalance,
    GROUP_CONCAT(DISTINCT fp.payment_status ORDER BY fp.payment_date DESC SEPARATOR ', ') as PaymentStatuses,
    MAX(fp.payment_date) as LastPaymentDate,
    COUNT(fp.payment_id) as TotalPaymentRecords
FROM student_profile sp
INNER JOIN fines_payments fp ON sp.students_id = fp.students_id
INNER JOIN fines f ON fp.fines_id = f.fines_id
LEFT JOIN registration r ON f.registration_no = r.registration_no
WHERE 1=1 $payment_where";
...
$sql .= " GROUP BY sp.students_id, sp.LastName, sp.FirstName, sp.Course, sp.Section, sp.YearLevel, fp.payment_type
          ORDER BY MAX(fp.payment_date) DESC, sp.LastName ASC, sp.FirstName ASC";
...
$stats_sql = "SELECT 
    COUNT(DISTINCT fp.students_id) as total_students,
    SUM(fp.payment_amount) as total_payments,
    COUNT(fp.payment_id) as total_records
FROM fines_payments fp
INNER JOIN student_profile sp ON fp.students_id = sp.students_id
INNER JOIN fines f ON fp.fines_id = f.fines_id
LEFT JOIN registration r ON f.registration_no = r.registration_no
WHERE 1=1 $payment_where";
...
<title>Payment Records | CSSO</title>
...
<div class="container">
    ...
    <div style="margin-bottom: 20px;">
        <div class="stat-card" style="max-width: 300px;">
            ...
            <p>₱<?= number_format($stats['total_payments'] ?? 0, 2) ?></p>
        </div>
    </div>
    <div class="controls">
            ...
            <table>
                ...
                <?php if ($result && $result->num_rows > 0): ?>
                    ...
                    <tr>
                        ...
                        <td><span class="badge badge-payment-type"><?= htmlspecialchars($row['payment_type']) ?></span></td>
                        <td>
                            <?php 
                            $balance = floatval($row['TotalBalance']);
                            $balanceClass = $balance <= 0 ? 'zero' : 'partial';
                            ?>
                            <span class="balance-badge <?= $balanceClass ?>">₱<?= number_format($balance, 2) ?></span>
                        </td>
                        <td>
                            <?php 
                            $statuses = explode(', ', $row['PaymentStatuses']);
                            foreach(array_unique($statuses) as $status):
                                $statusClass = $status === 'Paid' ? 'status-paid' : 'status-partial';
                            ?>
                                <span class="status-badge <?= $statusClass ?>"><?= htmlspecialchars($status) ?></span>
                            <?php endforeach; ?>
                        </td>
                        <td><?= date('M d, Y', strtotime($row['LastPaymentDate'])) ?></td>
                    </tr>
                    <?php endwhile; ?>
                <?php else: ?>
                    <tr>
                        <td colspan="9">
                            <div class="empty-state">
                                ...
                                <p>No payment records matching your search criteria.</p>
                            </div>
                        </td>
                    </tr>
                <?php endif; ?>
...
function loadSavedFilters() {
    ...
    const savedSemester = localStorage.getItem('paymentsSemesterFilter');
    ...
    const savedSchoolYear = localStorage.getItem('paymentsSchoolYearFilter');
...
function saveSemesterFilter() {
    ...
    if (semesterValue) {
        localStorage.setItem('paymentsSemesterFilter', semesterValue);
    } else {
        localStorage.setItem('paymentsSemesterFilter', 'all');
    }
}
function saveSchoolYearFilter() {
    const schoolYearValue = document.getElementById('schoolYearFilter').value;
    if (schoolYearValue) {
        localStorage.setItem('paymentsSchoolYearFilter', schoolYearValue);
    } else {
        localStorage.setItem('paymentsSchoolYearFilter', 'all');
    }
}
function clearFilters() {
    localStorage.removeItem('paymentsSemesterFilter');
    localStorage.removeItem('paymentsSchoolYearFilter');
